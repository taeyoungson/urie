...
def shot_noise(x, severity=1):
    ...
    x = np.array(x) / 255.
    return np.clip(np.random.poisson(x * c).astype(np.float) / c, 0, 1) * 255
...
def fog(x, severity=1):
    ...
    if max(h,w) <= 256:
        ...
        mapsize = 1024
    x += c[0] * plasma_fractal(mapsize, wibbledecay=c[1])[:h, :w][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    filename = ['./imagenet_dataset/frost1.png', './imagenet_dataset/frost2.png', './imagenet_dataset/frost3.png', './imagenet_dataset/frost4.jpg', './imagenet_dataset/frost5.jpg', './imagenet_dataset/frost6.jpg'][idx]
    ...
    # h, w, _ = x.shape
    w, h = x.size
    frost_h, frost_w, _ = frost.shape
    max_h = max(h, frost_h)
    max_w = max(w, frost_w)
    frost = cv2.resize(frost, (max_w, max_h))
    x_start = np.random.randint(0, frost.shape[0] - h) if frost.shape[0] - h else 0
    y_start = np.random.randint(0, frost.shape[1] - w) if frost.shape[1] - w else 0
...
def pixelate(x, severity=1):
    ...
    w, d = x.size
    # PILImage
    # x = x.resize((int(w * c), int(d * c)), PILImage.BOX)
    # x = x.resize((w, d), PILImage.BOX)
    x = x.resize((int(w * c), int(d * c)), PILImage.BILINEAR)
    x = x.resize((w, d), PILImage.BILINEAR)

    # cv2
    # x = cv2.resize(x, (int(w * c), int(h * c)), cv2.INTER_NEAREST)
    # x = cv2.resize(x, (w, h))
